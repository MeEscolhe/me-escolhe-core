...
router.put('/:id', async (req, res) => 
{
  //const { error } = valSkill(req.body); 
  //if (error) return res.status(400).send(error.details[0].message);
  const skill = await Skill.findByIdAndUpdate((mongoose.Types.ObjectId(req.params.id)), 
  { 
    languages: req.body.languages,
    soft: req.body.soft,
    hard: req.body.hard
...
router.delete('/:id', async (req, res) => {
  const skill = await Skill.findByIdAndRemove((mongoose.Types.ObjectId(req.params.id)));
