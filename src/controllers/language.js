...
router.get('/:id', async (req, res) => {
  ...
  const language = await Language.findById((mongoose.Types.ObjectId(req.params.id)));
...
router.put('/:id', async (req, res) => {
  ...
  const language = await Language.findByIdAndUpdate((mongoose.Types.ObjectId(req.params.id)), 
  { 
    name: req.body.name,
    level: req.body.level
  }, 
  ...
  if (!language) 
  {
    return res.status(404).send('The language with the given ID was not found.');
  } 
...
router.delete('/:id', async (req, res) => {
  const language = await Language.findByIdAndRemove((mongoose.Types.ObjectId(req.params.id)));
